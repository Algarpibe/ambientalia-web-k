...
### ESTADO DE `arquetipos` — LA ÚNICA LECTURA, y se DERIVA

> **`arquetipos` = 4 filas · 0 lectores.** Derivado el **2026-09-03 (144.ª)**,
> no recordado:
>
> | mitad | cómo se deriva | resultado |
> |---|---|---|
> | filas | `psql -c "select count(*) from arquetipos"` contra `kunak-cms-pg` | **4** |
> | lectores | `grep -rn "arquetipos" apps/web/src` | **0** — las 5 coincidencias son comentarios que usan la palabra en su sentido genérico (arquetipos de página), ninguna importa la colección |
>
> **Y por qué 0 lectores no es un pendiente sino un corte:** §regla 52 —la
> intersección entre lo que el modelo EXPRESA y lo que el render CONSUME es
> **cero** (22 constantes contra 31 campos), así que cablear el lector no es
> cambiar de fuente: es reescribir el arquetipo. Decisión del propietario,
> `PENDIENTES-QA.md` §141.ª.
>
> ⚠⚠ **ESTA ES LA ÚNICA LECTURA DE ESTADO DE `arquetipos` EN ESTE DOCUMENTO.**
> Había **seis** conviviendo —dos diciendo «0 filas», dos diciendo «4 filas», y
> la tabla de estado congelada en la 126.ª—, todas ciertas el día que se
> escribieron y **cuatro de ellas falsas hoy**. Las lecturas viejas quedan
> **BORRADAS, no anotadas** (§*una lectura se BORRA, nunca se concilia con una
> nota al pie*: mientras las dos estén escritas, cada lector elige la suya). Lo
> que se conserva de cada bloque `✏️ ACTUALIZADO` es **lo medido**, que no
> caduca; lo que se quitó es **el estado**, que caduca en cuanto alguien siembra.
>
> **Y la regla operativa, para que no vuelva a haber seis:** un bloque
> `✏️ ACTUALIZADO` registra **lo que esa tanda midió** (evento, con su fecha).
> **El estado va aquí y sólo aquí**, y se re-deriva al leerlo — no se cita.
...
> ✏️ **ACTUALIZADO 2026-08-31 (131.ª) — el motivo por el que no se sembraba,
> NUEVO y medido.** *(La lectura de estado que llevaba esta cabecera —«sigue
> siendo 0 filas · 0 lectores»— está BORRADA, no anotada: era cierta el
> 2026-08-31 y hoy es falsa. El estado vive en UN solo sitio, §ESTADO DE
> `arquetipos`, arriba.)*
>
...
> ✏️ **ACTUALIZADO 2026-09-02 (141.ª) — el motivo de los 0 lectores deja de ser
> un bloqueo de siembra y pasa a ser un CORTE LIMPIO DE MODELO.** *(El estado
> —«4 filas · 0 lectores»— vive en §ESTADO DE `arquetipos`, arriba, y no se
> repite aquí: lo que este bloque registra es POR QUÉ, que no caduca.)*
>
...
> ✏️ **ACTUALIZADO 2026-09-01 (132.ª) — la precondición pasa a DECIDIBLE en vez
> de sólo fichada.** *(La lectura de estado que iba aquí —«`arquetipos` a 0
> filas · 0 lectores»— está BORRADA: era cierta ese día y hoy es falsa. El
> estado vive en §ESTADO DE `arquetipos`, arriba.)*
>
> **Lo que la 132.ª NO hizo, y por encargo:** el sembrador queda escrito con su
> negativo verde y la siembra parada. **No se sembró, no se cableó y no se
> construyó**: la 132.ª es OFFLINE.
>
...
> ✏️ **ACTUALIZADO 2026-09-02 (140.ª) — la siembra ocurre: es la tanda que
> pone las 4 filas.** *(La transición «0 filas → 4 filas» que iba en esta
> cabecera se queda como EVENTO de la 140.ª; la lectura de ESTADO vive en
> §ESTADO DE `arquetipos`, arriba, y no se repite aquí.)*
>
