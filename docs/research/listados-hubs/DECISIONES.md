...
| **P-LH-C11** | ⛔ **NUEVA 2026-08-17 (75.ª tanda). Antes de fichar una indeterminación, comprueba que las dos hipótesis sean DISTINTAS.** La ventana de la piel B parecía tener dos predicados candidatos para el número grande —`L ≥ fin+3` y `L ≥ n+5`— y contar sus separadoras habría dado **0**, o sea la ficha de un dilema inexistente: son **la misma función** (en la ventana sin recortar `fin = n+2`, y en los recortes coinciden **porque `L` es múltiplo de 10 y nunca vale 6 ni 7**). §*dos modelos que predicen lo mismo son uno solo* tiene esta cara: a veces no hay que elegir, hay que **demostrar que no hay elección** — y eso se escribe, porque «0 separadoras» leído sin comprobarlo manda a la tanda siguiente a medir algo que no existe |
| **P-LH-C12** | ⛔ **NUEVA 2026-08-17. Toda tabla derivada de `piezas` del espejo declara el TOPE del instrumento.** `lh-barrido.mjs` congela `as.slice(0, 12)` y **2 de las 43** instancias de la piel B emiten **14**: una tabla de secuencia leída del espejo da esas dos filas *sin* `»` ni `Last »` y **parece el original**. Los canales sin truncar son `paginador.hrefs` (del mismo fichero) y el HTML del corpus. Vale para cualquier arquetipo futuro cuyo componente tenga más de 12 piezas |
