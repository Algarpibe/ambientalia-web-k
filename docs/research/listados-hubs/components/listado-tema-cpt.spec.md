...
> | ~~**SP-C10**~~ | §3 describe las piezas de la piel B y **no su VENTANA** | ✅ **COMPLETADA 2026-08-17 (75.ª).** **5 números** con `« First` · `...` · `Last »`, **más el NÚMERO GRANDE** (`a.larger.page`, múltiplo de 10) cuando `L ≥ fin + 3`, entre sus dos `...` y delante de `»`/`Last »`. Derivado de las **43** instancias: el componente reproduce **43/43** secuencias enteras, el que había **7/43**. ⚠ El `38/38` que aquí se citaba **excluía del denominador las 5 instancias con número grande** — o sea 377 pares de defecto dentro de un verde. Y las instancias de esta spec (`total = 4`) **no podían separar los dos modelos** |
