...
| # | decisión | bloquea |
|---|---|---|
| **CMS-ORDEN-L2** | **¿cómo ordena el clon un ARCHIVO DE CPT?** El original ordena `/glosario` por **`datePublished` DESC (37/37)** y **ninguno de los dos tipos del clon tiene campo de fecha**. Dos mitades distintas, y **no se resuelven con la misma decisión** — ver §7e. ⚠ **AMPLIADA 2026-08-17 (75.ª): la pregunta no es sólo *«por qué campo»*, es también *«con qué DESEMPATE»*.** Medido en los listados **ya construidos**: un grupo de **36 pares de contenido** aparece y desaparece entre builds del **mismo** código, y la forma que toma es **la ventana de la página desplazada una posición** — o sea que el corte entre páginas no cae siempre en el mismo sitio. Un `orderBy` sin criterio de desempate **total y estable** produce exactamente eso. Ficha con sus tres corridas: `PENDIENTES-QA.md` §F3-LH-LISTADO-QUE-OSCILA | **`L2` entera** (12 rutas) **y la estabilidad de los listados ya emitidos** |
